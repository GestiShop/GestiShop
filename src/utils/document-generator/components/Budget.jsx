...
const BudgetHeader = ({
  ...
  const budgetNumberToPrint = budgetNumberPreamble
    ? `${budgetNumberPreamble}-${budgetNumber}`
    : budgetNumber;
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={6}>
        <Typography align="right" className="mb-2r bold" paragraph>
          {`${t('accounting_module.budget.structure.date')}: ${moment(
            date
          ).format('DD/MM/YYYY')}`}
        </Typography>
    ...
    <TableContainer>
      <Table>
        ...
        <TableBody>
          ...
          <TableRow>
            <StyledTableCell>
              {t('accounting_module.product.document.total_budget', {
                currency,
              })}
            </StyledTableCell>
            <StyledTableCell align="right" colSpan={2}>
              {budgetTotal.toFixed(numberOfDecimals)}
            </StyledTableCell>
          </TableRow>
        </TableBody>
      </Table>
...
export { BudgetHeader, ProductsTable, SummaryTable };
