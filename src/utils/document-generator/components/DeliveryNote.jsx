...
const DeliveryNoteHeader = ({
  ...
  const deliveryNoteNumberToPrint = deliveryNoteNumberPreamble
    ? `${deliveryNoteNumberPreamble}-${deliveryNoteNumber}`
    : deliveryNoteNumber;
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={6}>
        <Typography align="right" className="mb-2r bold" paragraph>
          {`${t('accounting_module.delivery_note.structure.date')}: ${moment(
            date
          ).format('DD/MM/YYYY')}`}
        </Typography>
...
const SummaryTable = ({ products }) => {
  ...
  const deliveryNoteTotal = products
    .map(
      (product) =>
        product.basePricePerUnit *
        product.quantity *
...
export { DeliveryNoteHeader, ProductsTable, SummaryTable };
